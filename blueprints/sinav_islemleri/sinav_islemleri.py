...
from models import Ogrenci, OgretimElemani, Sinav, DersAcma, DersAlma, DersHavuzu, Mufredat, db, Derslik, Bolum
        ...
        try:
            ...
            if redirect_user(decoded_token.get('user_type'), 'ogrenci'):
                ...
                active_semester_exams = (
                    ...
                    .join(Ogrenci, Bolum.BolumID == Ogrenci.BolumID)
                    .join(DersAlma, Ogrenci.OgrenciID == DersAlma.OgrenciID)
                    .join(Sinav, DersAlma.DersAlmaID == Sinav.DersAlmaID)
                    .join(Mufredat, DersAcma.MufredatID == Mufredat.MufredatID)
                    .join(DersHavuzu, Mufredat.DersID == DersHavuzu.DersID)
                    .filter(
                        Ogrenci.BolumID == user.BolumID,
                        DersAlma.AkademikYil == donem,  # Adjust the academic year as needed
                    )
                    .order_by(desc(Sinav.SinavTarihi),
                              desc(Sinav.SinavSaati))  # Order by exam date and time in descending order
                    .all()
